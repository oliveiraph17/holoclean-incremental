...
class Dataset:
    ...
    def get_repaired_dataset(self):
        ...
        for tid in repaired_vals:
            for attr in repaired_vals[tid]:
                # Updates the value if it was repaired.
                if init_records[tid][attr] != repaired_vals[tid][attr]:
    ...
    def get_repaired_dataset_incremental(self):
        ...
        if self.is_first_batch():
            # Gets only rows from the current batch for repairing and use _tid_ to index init_records.
            init_records = self.raw_data.df.sort_values(['_tid_']).to_records(index=False)

        else:
            # In this case we have to create a map _tid_ to idx to index init_records.
            if self.repair_previous_errors:
                ...
                # Drops the old index [1..n,1..m], generated by concatenating the two Data Frames of sizes n and m,
                # and creates a new one [1..n+m].
            ...
            # Creates a dictionary mapping _tid_ to index.
