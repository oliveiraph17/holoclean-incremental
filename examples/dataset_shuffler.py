...
class DatasetShuffler:
    ...
    def load_data(self, name, fpath, entity_col=None,
                  store_to_db=False, shuffle=False, sort_by=None, ascending=True):
        """
        ...
        # Rearranges the dataset according to the input parameters.
...
# Generates a new CSV with tid in ascending order (like HoloClean does when the input file has no tid attribute).
gen.load_data('hospital', '../testdata/hospital/hospital')

# Generates a new CSV with the same tid's of the input file, but shuffled.
# gen.load_data('hospital', '../testdata/hospital/hospital', shuffle=True)

# Generates a new CSV with the same tid's of the input file but sorted according to the provided list.
# gen.load_data('hospital', '../testdata/hospital/hospital', sort_by=['MeasureName', 'Condition'])
